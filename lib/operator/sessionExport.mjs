 ...
 */
export function formatExportConfirmation(method, format = 'markdown') {
  const formatLabel = format === 'json' ? 'JSON' : 'Markdown';
  if (method === 'share') {
    return [
      'NEXIFY EXPORT',
      '',
      `SESSION exportovaná ako ${formatLabel}.`,
...
/**
 * @param {Array<{ role?: string; content?: string; type?: string; createdAt?: string | number }>} messages
 * @param {{ exportedAt?: Date; pin?: string; session?: Record<string, unknown> }} [options]
 */
export function formatSessionJson(messages = [], options = {}) {
  const exportedAt = options.exportedAt instanceof Date ? options.exportedAt : new Date();
  const pin = options.pin ?? DEFAULT_REDACT_PIN;
  const list = Array.isArray(messages) ? messages : [];
      ...
      messages: list.map((msg, index) => ({
        ...
        createdAt:
          msg?.createdAt != null
            ? new Date(msg.createdAt).toISOString()
            : null,
        content: redactExportSecrets(stripAnsiForSession(msg?.content ?? ''), { pin }),
      })),
    },
    null,
    2,
...
/**
 * @param {string} text
 * @param {{ navigator?: Navigator; title?: string }} [options]
 * @returns {Promise<{ method: 'share' | 'clipboard' }>}
 */
export async function deliverSessionText(text, options = {}) {
  const nav = options.navigator ?? globalThis.navigator;
  const payloadText = String(text || '');
  const title = options.title || 'Nexify Session';
  ...
  if (nav?.share) {
    const payload = { title, text: payloadText };
  ...
  await nav.clipboard.writeText(payloadText);
...
export async function deliverSessionMarkdown(markdown, scope = globalThis) {
  return deliverSessionText(markdown, {
    navigator: scope.navigator,
    title: 'Nexify Session',
  });
}